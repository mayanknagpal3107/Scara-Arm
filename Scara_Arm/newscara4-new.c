...
int main(void)
{
	...
	DDRC |= (1<<PC3)|(1<<PC2)|(1<<PC4)|(1<<PC1);	// initialize servo
	...
	while(1)
		{
			...
			if(c==0xff)
			{
				...
				if(c==0x65)
				{
					...
					uart_puts(" \n\rAngle:\n\r");
					serial_data[1]=uart_getc();
					_delay_ms(2);
					serial_data[2]=uart_getc();
					do
					{
						c=uart_getc();
					}while(abs(c-0xff));
					uart_puts(" \n\rOk\n\r");
					setServoPosition(1,serial_data[1]);
					setServoPosition(2,serial_data[2]);
					wdt_reset();
				}
				/*
			else if(c==0x66)
				{
					uart_puts(" \n\rAngle:\n\r");
					c=uart_getc();
					setServoPosition(1,c);
					c=uart_getc();
					setServoPosition(2,c);
					wdt_reset();
				}
				*/
			}
